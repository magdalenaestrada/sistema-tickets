...
<head>
    <meta charset="UTF-8">
    <title>Corte masivo de cajas</title>
    <style>
        body {
...
<body>
    ...
    @foreach ($cajas as $caja)
        <div class="text-center mb-3">
            ...
            <div>{{ $caja->sucursal->nombre ?? 'Sucursal' }}</div>
            <div>CORTE DE CAJA #{{ $caja->id }}</div>
        </div>

        <table class="no-border mb-3">
            <tr>
                <td><strong>Caja:</strong> #{{ $caja->id }}</td>
                <td><strong>Estado:</strong> {{ in_array($caja->estado, ['C', 'cerrada']) ? 'CERRADA' : 'ABIERTA' }}
                </td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td><strong>Fecha cierre:</strong>
                    {{ $caja->fecha_cierre ? optional($caja->fecha_cierre)->format('d/m/Y h:i A') : '---' }}</td>
            </tr>
        </table>
